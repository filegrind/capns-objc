...
- (void)testCapGraphBasicConstruction {
    ...
    // binary -> str -> obj
    CSCap *cap1 = [self makeGraphCapWithInSpec:@"std:binary.v1" outSpec:@"std:str.v1" title:@"Binary to String"];
    CSCap *cap2 = [self makeGraphCapWithInSpec:@"std:str.v1" outSpec:@"std:obj.v1" title:@"String to Object"];
...
- (void)testCapGraphOutgoingIncoming {
    ...
    CSCap *cap1 = [self makeGraphCapWithInSpec:@"std:binary.v1" outSpec:@"std:str.v1" title:@"Binary to String"];
    CSCap *cap2 = [self makeGraphCapWithInSpec:@"std:binary.v1" outSpec:@"std:obj.v1" title:@"Binary to Object"];
...
- (void)testCapGraphCanConvert {
    ...
    // Direct conversions
    XCTAssertTrue([graph canConvert:@"std:binary.v1" toSpec:@"std:str.v1"], @"Should convert binary to str");
    XCTAssertTrue([graph canConvert:@"std:str.v1" toSpec:@"std:obj.v1"], @"Should convert str to obj");

    // Transitive conversion
    XCTAssertTrue([graph canConvert:@"std:binary.v1" toSpec:@"std:obj.v1"], @"Should convert binary to obj transitively");

    // Same spec
    XCTAssertTrue([graph canConvert:@"std:binary.v1" toSpec:@"std:binary.v1"], @"Should convert same spec");

    // Impossible conversions
    XCTAssertFalse([graph canConvert:@"std:obj.v1" toSpec:@"std:binary.v1"], @"Should not convert obj to binary");
    XCTAssertFalse([graph canConvert:@"std:nonexistent.v1" toSpec:@"std:str.v1"], @"Should not convert nonexistent");
}
...
- (void)testCapGraphFindPath {
    ...
    // Direct path
    NSArray<CSCapGraphEdge *> *path = [graph findPath:@"std:binary.v1" toSpec:@"std:str.v1"];
    XCTAssertNotNil(path, @"Should find path from binary to str");
    XCTAssertEqual(path.count, 1, @"Expected path length 1");

    // Transitive path
    path = [graph findPath:@"std:binary.v1" toSpec:@"std:obj.v1"];
    ...
    // No path
    path = [graph findPath:@"std:obj.v1" toSpec:@"std:binary.v1"];
    XCTAssertNil(path, @"Should not find impossible path");

    // Same spec
    path = [graph findPath:@"std:binary.v1" toSpec:@"std:binary.v1"];
...
- (void)testCapGraphFindAllPaths {
    ...
    CSCap *cap1 = [self makeGraphCapWithInSpec:@"std:binary.v1" outSpec:@"std:str.v1" title:@"Binary to String"];
    CSCap *cap2 = [self makeGraphCapWithInSpec:@"std:str.v1" outSpec:@"std:obj.v1" title:@"String to Object"];
    CSCap *cap3 = [self makeGraphCapWithInSpec:@"std:binary.v1" outSpec:@"std:obj.v1" title:@"Binary to Object (direct)"];
    ...
    NSArray<NSArray<CSCapGraphEdge *> *> *paths = [graph findAllPaths:@"std:binary.v1" toSpec:@"std:obj.v1" maxDepth:3];
...
- (void)testCapGraphGetDirectEdges {
    ...
    CSCap *cap1 = [self makeGraphCapWithInSpec:@"std:binary.v1" outSpec:@"std:str.v1" title:@"Generic Binary to String"];

    // More specific converter (with extra tag for higher specificity)
    CSCapUrn *capUrn2 = [CSCapUrn fromString:@"cap:ext=pdf;in=std:binary.v1;op=convert;out=std:str.v1" error:nil];
    ...
    NSArray<CSCapGraphEdge *> *edges = [graph getDirectEdges:@"std:binary.v1" toSpec:@"std:str.v1"];
...
- (void)testCapGraphStats {
    ...
    CSCap *cap1 = [self makeGraphCapWithInSpec:@"std:binary.v1" outSpec:@"std:str.v1" title:@"Binary to String"];
    CSCap *cap2 = [self makeGraphCapWithInSpec:@"std:str.v1" outSpec:@"std:obj.v1" title:@"String to Object"];
    CSCap *cap3 = [self makeGraphCapWithInSpec:@"std:binary.v1" outSpec:@"std:json.v1" title:@"Binary to JSON"];
...
- (void)testCapGraphWithCapCube {
    ...
    // Provider: binary -> str
    CSCap *providerCap = [self makeGraphCapWithInSpec:@"std:binary.v1" outSpec:@"std:str.v1" title:@"Provider Binary to String"];
    [providerRegistry registerCapSet:@"provider" host:providerHost capabilities:@[providerCap] error:nil];

    // Plugin: str -> obj
    CSCap *pluginCap = [self makeGraphCapWithInSpec:@"std:str.v1" outSpec:@"std:obj.v1" title:@"Plugin String to Object"];
    ...
    XCTAssertTrue([graph canConvert:@"std:binary.v1" toSpec:@"std:obj.v1"], @"Should convert across registries");

    NSArray<CSCapGraphEdge *> *path = [graph findPath:@"std:binary.v1" toSpec:@"std:obj.v1"];
