...
- (void)testRegisterAndFindCapSet {
    ...
    // Test no match
    sets = [self.registry findCapSets:@"cap:in=media:void;op=different;out=media:object" error:&findError];
...
- (void)testBestCapSetSelection {
    ...
    CSCapUrn *generalCapUrn = [CSCapUrn fromString:@"cap:in=media:void;op=generate;out=media:object" error:nil];
...
- (void)testCanHandle {
    // Empty registry
    BOOL canHandle = [self.registry canHandle:@"cap:in=media:void;op=test;out=media:object"];
    ...
    CSCapUrn *capUrn = [CSCapUrn fromString:@"cap:in=media:void;op=test;out=media:object" error:nil];
    CSCap *cap = [CSCap capWithUrn:capUrn
                             title:@"Test"
                           command:@"test"
                       description:@"Test"
    ...
    canHandle = [self.registry canHandle:@"cap:in=media:void;op=test;out=media:object"];
    XCTAssertTrue(canHandle, @"Registry should handle registered capability");

    canHandle = [self.registry canHandle:@"cap:extra=param;in=media:void;op=test;out=media:object"];
    XCTAssertTrue(canHandle, @"Registry should handle capability with extra parameters");

    canHandle = [self.registry canHandle:@"cap:in=media:void;op=different;out=media:object"];
...
- (void)testUnregisterCapSet {
    ...
    XCTAssertTrue([self.registry canHandle:@"cap:in=media:void;op=test;out=media:object"], @"Should handle capability before unregistering");
    ...
    // Verify it's gone
    XCTAssertFalse([self.registry canHandle:@"cap:in=media:void;op=test;out=media:object"], @"Should not handle capability after unregistering");
...
- (void)testClear {
    ...
    // Verify it's registered
    XCTAssertTrue([self.registry canHandle:@"cap:in=media:void;op=test;out=media:object"], @"Should handle capability before clearing");
    ...
    XCTAssertFalse([self.registry canHandle:@"cap:in=media:void;op=test;out=media:object"], @"Should not handle any capabilities after clearing");
